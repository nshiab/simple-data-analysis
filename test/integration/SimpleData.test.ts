...
async function main() {
    ...
    const simpleDataArray = new SimpleData({ verbose: true, logParameters: true })
        .loadDataFromArray({
            data: [{ first: "Nael", last: "Shiab" }, { first: "Isabelle", last: "Bouchard" }]
        })

    const simpleDataWeb = await new SimpleData({ verbose: true, logParameters: true })
    ...
    const simpleData = new SimpleData({ verbose: true, logParameters: true })
        .loadDataFromLocalFile({ path: "./data/employees.csv" })
