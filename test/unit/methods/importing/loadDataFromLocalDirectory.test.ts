...
describe("loadDataFromLocalDirectory", function () {
    ...
    it("should return an array of objects from a directory with text files without headers and a specific format", function () {
        const sd = new SimpleDataNode().loadDataFromLocalDirectory({
            path: "./test/data/wholeDirectoryTextNoHeaders/",
            format: "csv",
            headers: ["key1", "key2"],
        })
        assert.deepStrictEqual(sd.getData(), [
