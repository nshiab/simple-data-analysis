...
Deno.test("should normalize strings that share the same prefix when prefixBlockingSize is set", async () => {
  const sdb = new SimpleDB();
  const table = sdb.newTable();
  // "New York" and "New Yorkk" share the prefix "New" (3 chars) — they should still be merged.
  await table.insertRows([
    { city: "New York" },
    { city: "New York" },
    { city: "New Yorkk" },
  ...
  await table.fuzzyClean("city", "city", { prefixBlockingSize: 3 });

  const data = await table.getData();

  // "New York" (count=2) wins over "New Yorkk" (count=1)
  // "Paris" (count=2) wins over "Pariss" (count=1)
  assertEquals(data, [
    { city: "New York" },
    { city: "New York" },
    { city: "New York" },
...
Deno.test("should not merge strings that differ in their leading characters when prefixBlockingSize is set", async () => {
  const sdb = new SimpleDB();
  const table = sdb.newTable();
  // "MacDonald" and "McDonald" have a ratio above 80 and would normally be merged,
  // but LEFT("MacDonald", 3) = "Mac" ≠ LEFT("McDonald", 3) = "McD" — blocking prevents the comparison.
  await table.insertRows([
    { name: "MacDonald" },
    { name: "McDonald" },
  ]);

  await table.fuzzyClean("name", "name", { prefixBlockingSize: 3 });

  const data = await table.getData();

  // Prefix mismatch → no candidate pair generated → both values left unchanged
  assertEquals(data, [
    { name: "MacDonald" },
    { name: "McDonald" },
  ]);
