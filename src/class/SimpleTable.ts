...
export default class SimpleTable extends SimpleTableCore {
   ...
   *
   * Embeddings are cached locally in `.journalism-cache` by default. Set `embeddings.cache` to `false` to disable caching, and remember to add `.journalism-cache` to your `.gitignore`.
   *
   ...
   *
   * The query embedding is cached locally in `.journalism-cache` by default. Set `embeddings.cache` to `false` to disable caching, and remember to add `.journalism-cache` to your `.gitignore`.
   *
   ...
   * Also, the method creates the column `{columnText}_embeddings` to store the generated embeddings and persists its canonical embedding provenance inside DuckDB. A stored column is reused only when its provider/backend/model identity, semantic options, source mapping, and dimensions remain compatible. Legacy or incompatible columns are regenerated, and stale vector indexes are invalidated before replacement. This provenance survives reopening a DuckDB database.
   *
   * Generation and embeddings are cached by default. To delete the cache, remove the `.journalism-cache` and/or `.sda-cache` directories, or set `generation.cache` and/or `embeddings.cache` to `false`.
   *
