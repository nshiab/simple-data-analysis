...
export default class SimpleTable extends SimpleTableCore {
   ...
   *
   * If `createIndex` is `true`, an HNSW index will be created on the new column using the [duckdb-vss extension](https://github.com/duckdb/duckdb-vss). This is useful for speeding up the `aiVectorSimilarity` method. If the index already exists, it will not be recreated unless `overwriteIndex` is `true`. The physical index is not stored in DuckDB cache artifacts; it is recreated from `SimpleTable.indexes` on cache hits.
   *
   * This method does not support tables containing geometries.
   *
   ...
   *
   * If `createIndex` is `true`, an HNSW index will be created on the embeddings column using the [duckdb-vss extension](https://github.com/duckdb/duckdb-vss) to speed up processing. If the index already exists, it will not be recreated unless `overwriteIndex` is `true`. The physical index is not stored in DuckDB cache artifacts; it is recreated from `SimpleTable.indexes` on cache hits.
   *
   ...
   *
   * When BM25 search is enabled, its required full-text search index is created or reused automatically. The physical FTS index is stored in DuckDB cache artifacts and restored directly on cache hits.
   *
   * When vector search is enabled, set `createIndex` to `true` to also create an HNSW index using the [duckdb-vss extension](https://github.com/duckdb/duckdb-vss). The physical HNSW index is not stored in cache artifacts; it is recreated from `SimpleTable.indexes` on cache hits.
   *
   * This method does not support tables containing geometries.
   *
